...
#compile
if (OS_WIN)
	set(X_OPTIMIZATION_NONE_FLAGS " /Od /Ob0 ")
	set(X_OPTIMIZATION_MAXSPEED_FLAGS " /O2 /Ob2 /Ot ")

	set(CMAKE_C_FLAGS  " /W3 /WX- /Zm500 /nologo /ZI /Gm- /Oy- /GS /Zc:wchar_t /Gd /GR ")
	set(CMAKE_C_FLAGS_DEBUG " /D_DEBUG /RTC1 ${X_OPTIMIZATION_NONE_FLAGS} ")
	set(CMAKE_C_FLAGS_RELEASE " /DNDEBUG /MP ${X_OPTIMIZATION_MAXSPEED_FLAGS} ")

	set(CMAKE_CXX_FLAGS " ${CMAKE_C_FLAGS} /EHsc ")
	set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_C_FLAGS_DEBUG}")
	set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE}")

	set(CMAKE_EXE_LINKER_FLAGS_RELEASE " /DEBUG /INCREMENTAL:NO /OPT:REF /OPT:ICF ${CMAKE_EXE_LINKER_FLAGS_RELEASE}")
	set(CMAKE_EXE_LINKER_FLAGS_DEBUG " /DEBUG /INCREMENTAL ${CMAKE_EXE_LINKER_FLAGS_DEBUG}")
endif()
	...
	if(NOT XG_ENV_PATH)
		if(OS_WIN)
			get_filename_component(
				XG_ENV_PATH 
				"[HKEY_CURRENT_USER\\Software\\bigo\\dev_evn;qt]"
				ABSOLUTE 
				)
			if(NOT XG_ENV_PATH)
...
macro(_get_target_impt_lib _tgt_name _impt_lib)
	...
	if("${_tgt_type}" STREQUAL "EXTERN")
		...
		if(OS_WIN)
			set(${_impt_lib} "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_${XG_BUILD_CFG}}/${_tgt_name}.lib")
		elseif(OS_LINUX)
			if("${_tgt_type}" STREQUAL "SHARED")
				set(${_impt_lib} "${CMAKE_LIBRARY_OUTPUT_DIRECTORY_${XG_BUILD_CFG}}/lib${_tgt_name}.so")
			elseif("${_tgt_type}" STREQUAL "STATIC")
				set(${_impt_lib} "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_${XG_BUILD_CFG}}/$lib{_tgt_name}.a")
			else()
				message(FATAL_ERROR "can't link to executable target ${_tgt_name}")
			endif()
		elseif(OS_MAC)
			if("${_tgt_type}" STREQUAL "SHARED")
				set(${_impt_lib} "${CMAKE_LIBRARY_OUTPUT_DIRECTORY_${XG_BUILD_CFG}}/lib${_tgt_name}.dylib")
			elseif("${_tgt_type}" STREQUAL "STATIC")
				set(${_impt_lib} "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_${XG_BUILD_CFG}}/$lib{_tgt_name}.a")
			else()
				message(FATAL_ERROR "can't link to executable target ${_tgt_name}")
...
macro(_deal_target_link)
	foreach(_tgt ${XG_TARGETS})
		get_property(_link_tgts TARGET ${_tgt} PROPERTY XT_LINK_INC_TARGETS)
		if (NOT _link_tgts)
			#continue()
		else()

			get_property(_tgt_type_ TARGET ${_tgt} PROPERTY XT_TARGET_TYPE)
			...
			foreach(_lp ${_link_tgts})
				...
				if(NOT "${_tgt_type_}!" STREQUAL "STATIC!")
				...
				#include the target
				get_property(_export_dir TARGET ${_lp} PROPERTY XT_HEADER_DIR)
				if (_export_dir)
					target_include_directories(${_tgt} PUBLIC ${_export_dir})
				endif()
			endforeach()
